...
export class MotoboysController {
  ...
  create(@Body() dto: CreateMotoboyDto, @Request() req) {
    ...
    return this.motoboysService.create({
      ...dto,
      userId: user.id, // associa o motoboy ao user autenticado
    });
  }
  ...
  @UseGuards(AuthGuard('jwt'), RolesGuard)
  @Roles(UserRole.USER, UserRole.ADMIN)
  update(@Param('id') id: string, @Body() dto: UpdateMotoboyDto, @Request() req) {
    const user = req.user;
    if (user.role !== UserRole.ADMIN && user.id !== +id) {
      return { message: 'Acesso negado: só pode atualizar a si mesmo' };
    }
