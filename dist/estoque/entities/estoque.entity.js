"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Estoque = void 0;
const typeorm_1 = require("typeorm");
const fruta_entity_1 = require("../../fruta/entities/fruta.entity");
const loja_entity_1 = require("../../loja/entities/loja.entity");
let Estoque = class Estoque {
    id;
    quantidade;
    fruta;
    loja;
};
exports.Estoque = Estoque;
__decorate([
    (0, typeorm_1.PrimaryGeneratedColumn)(),
    __metadata("design:type", Number)
], Estoque.prototype, "id", void 0);
__decorate([
    (0, typeorm_1.Column)('int'),
    __metadata("design:type", Number)
], Estoque.prototype, "quantidade", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(() => fruta_entity_1.Fruta, (fruta) => fruta.estoques),
    __metadata("design:type", fruta_entity_1.Fruta)
], Estoque.prototype, "fruta", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(() => loja_entity_1.Loja, (loja) => loja.estoques),
    __metadata("design:type", loja_entity_1.Loja)
], Estoque.prototype, "loja", void 0);
exports.Estoque = Estoque = __decorate([
    (0, typeorm_1.Entity)()
], Estoque);
//# sourceMappingURL=estoque.entity.js.map